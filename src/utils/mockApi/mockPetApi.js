import * as SwaggerPetstore from "../../stub/src/index"
import {mockFindPetsByStatus} from "./mockMethods/mockFindPetsByStatus"

let apiInstance = new SwaggerPetstore.PetApi()
apiInstance.findPetsByStatus = (status, callback) => {
	return new Promise((resolve, reject) => {
		if (status.every((item) => typeof item === "string") && callback) {
			const data = mockFindPetsByStatus()
			const response = 200
			const error = ""
			resolve(callback(error, data, response))
		} else {
			const data = []
			const response = 400
			const error = "метод findPetsByStatus прошел с ошибкой"
			reject(callback(error, data, response))
		}
	})
}

export const mockPetApi = apiInstance
